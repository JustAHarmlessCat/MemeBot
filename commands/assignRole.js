...
module.exports = {
    ...
    async execute(interaction = new CommandInteraction()) {
        ...
        if (!roles.some(r => r.name === role.name)) {
            await interaction.reply({ content: `Rolle ${role.name} existiert nicht.`, ephemeral: true });
            return;
        }

        const member = interaction.guild.members.cache.get(user.id);
        if (!role) {
            await interaction.reply({ content: `Diese Rolle ist keine Customrole daher darf sie nicht vergeben werden`, ephemeral: true });
            return;
        }

        await member.roles.add(role);
        await interaction.reply({ content: `Rolle ${role.name} wurde an ${user.username} vergeben.`, ephemeral: true });
    },
};